...
class CompaniesController {
    ...
    async createCompanies(request) {
        if (request.body.fromDate && request.body.toDate && request.body.fromDate > request.body.toDate) {
            throw new _http.BadRequestError('toDate must be greater or equal than fromDate');
        }
        ...
        if (industriesCount !== industriesIds.length) {
            throw new _http.NotFoundError('INDUSTRY_NOT_FOUND');
        }
        ...
        if (functionsCount !== functionsIds.length) {
            throw new _http.NotFoundError('FUNCTION_NOT_FOUND');
        }
        ...
        if (existingCompanies.length) {
            ...
            if (companiesCount !== companiesIds.length) {
                throw new _http.NotFoundError('COMPANY_NOT_FOUND');
            }
        ...
        return new _userProfessionals.UserProfessionalDto(userCompany);
    }
}
