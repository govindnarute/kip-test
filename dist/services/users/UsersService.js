...
`, {
            replacements: {
                ...
                hasLocations: query.locationsId && query.locationsId.length || false,
                locationsId: query.locationsId || [-1],
                hasIndustries: query.industriesId && query.industriesId.length || false,
                industriesId: query.industriesId || [-1],
                hasFunctions: query.functionsId && query.functionsId.length || false,
    ...
    async checkProfileCompleted(request, response, next) {
        const hasCompletedProfile = await this.dbConnection.query('SELECT hasCompletedProfile(:userId)', {
            replacements: { userId: request.user.userId },
            type: this.dbConnection.QueryTypes.SELECT
        });

        if (!hasCompletedProfile[0][`hasCompletedProfile(${request.user.userId})`]) {
            return next(new _http.ForbiddenError('ACCESS_DENIED'));
        }
    ...
    async checkUserVerification(request, response, next) {
        ...
        if (!user) {
            return next(new _http.NotFoundError('USER_NOT_FOUND'));
        }
    ...
    async checkUserIdentities(request, response, next) {
        ...
        if (!mask.isProfessional && !mask.isCoach) {
            return next(new _http.ForbiddenError('IDENTITY_ERROR'));
        }
