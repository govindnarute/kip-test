...
class SocialsService {
    ...
    async getSocialByTypeAndSocialUserId(type, socialUserId) {
        return this.Social.scope({ method: ['byType', type] }, { method: ['bySocialUserId', socialUserId] }, 'withUser').findOne();
    }
    ...
    async addSocialUser(socialUser, socialType, userType) {
        ...
        try {
            user = await User.create({
                ...
                type: userType ? userType : _UserTypes.userTypes.both,
                isVerified: true,
    ...
    async addImage(image, user) {
        ...
        try {
            const imageForSave = {
                authorId: user.id,
                name: image.name,
                status: _imagesStatuses.ImageStatus.Loaded
            };
...
exports.default = SocialsService;
//# sourceMappingURL=SocialsService.js.map